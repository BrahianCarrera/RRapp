...
with st.sidebar:
    ...
    if uploaded:
        ...
        if uploaded.name != st.session_state.original_filename or st.session_state.df is None:
    ...
    if st.session_state.df is not None:
        ...
        dl_name = os.path.splitext(st.session_state.original_filename)[0] + "_validado.csv"
        st.download_button(
    ...
    with st.form(key=f"form_{idx}"):
        ...
        if submitted:
            ...
            if not coincidencia:
                ...
                # ── Auto-save to disk ──────────────────────────────────────
                autosave()
                st.success(f"✅ Paciente **{row.get('subject_id','')}** guardado — archivo actualizado automáticamente.")
